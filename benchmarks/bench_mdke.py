#!/usr/bin/env python
"""mDKE convergence benchmark: run the case matrix, record deterministic solve
metrics, and diff two runs to catch regressions.

This is the monoenergetic sibling of ``bench_dke.py``. It is **not** a pytest test
(it takes a while or needs a GPU); it is a standalone regression / profiling tool.
It records only hardware-independent, deterministic quantities - matvec count
(``nmv``), restart count (``niter``), convergence flag (``success``) and final
residual (``res``). The mDKE solves two RHS per case (the D*1 and D*3 drives); the
recorded ``nmv``/``niter`` are the sums, ``success`` is the AND, and ``res`` is the
worse of the two. The per-RHS matvec counts are also stored (never compared).
Per-iteration timing is deliberately out of scope.

The solver is always run with whatever ``solve_mdke`` uses by default, so a change
in ``nmv``/``success`` is a change in the shipped code. A/B testing is done by the
two result files.
...
-----
    ...
    python benchmarks/bench_mdke.py compare baseline.json nightly.json

Typical regression check across a code change::

    git checkout main    && python benchmarks/bench_mdke.py run --out base.json
    git checkout feature && python benchmarks/bench_mdke.py run --out feat.json
    python benchmarks/bench_mdke.py compare base.json feat.json
"""
...
from yancc.solve import solve_mdke  # noqa: E402
...
# Descriptive columns (field / nuhat / erhat / grid) shared by run and compare so
# the case being solved is legible without cross-referencing cases_mdke.py.
_CASE_HDR = f"{'field':>12} {'nuhat':>10} {'erhat':>10} {'grid':>11}"
...
def _case_cols(case: Case) -> str:
    ...
    na, nt, nz = case.res
    grid = f"{na}x{nt}x{nz}"
    return f"{case.field:>12} {case.nuhat:>10.1e} {case.erhat:>10.2e} {grid:>11}"
...
def run_case(case: Case, verbose: int = 0) -> dict:
    ...
    return {
        "nmv": nmv1 + nmv2,
        "niter": int(info["j1"]) + int(info["j2"]),
        "success": bool(info["success1"]) and bool(info["success2"]),
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.case:
        ...
        label = args.tier
    try:
        cases, suffix = select_from_args(cases, "mdke", args)
...
def _ordered_names(names: set[str]) -> list[str]:
    """Order result names by their declaration order in ``cases_mdke.CASES``.
...
def main() -> int:
    ...
    pr.add_argument(
        "--verbose",
        action="store_true",
        help="pass verbose=2 to solve_mdke to monitor krylov progress per case",
    )
